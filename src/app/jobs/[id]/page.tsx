...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!detail) notFound();
  const { job, currentProfileVersion, runs, fit, resume, application } = detail;
  const hasStale = [fit, resume, application].some((result) => result?.isStale);
  const mockRun = ([
    ["fit", fit], ["resume", resume], ["application", application],
  ] as const).some(([kind, result]) => result && runs.find((run) => run.kind === kind && run.status === "complete" && run.profileVersion === result.profileVersion)?.provider === "mock");
  return <main className="page-shell detail-page"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/">Pipeline</Link><span aria-hidden="true">/</span><span>Job {job.id}</span></nav>
    ...
    {hasStale && <div className="stale-note global-stale" role="note"><strong>Some results use an older profile version.</strong> They are labeled below. Run or regenerate analysis to update them.</div>}
