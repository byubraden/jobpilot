...
export function FitAnalysisView({ result }: { result: { payload: FitAnalysis; profileVersion: number | null; isStale: boolean } | null }) {
  ...
  return <section className={`card result-card ${isStale ? "stale-result" : ""}`} aria-label={isStale ? "Previous fit analysis" : "Current fit analysis"}>
    <div className="section-heading compact"><div><p className="eyebrow">{isStale ? `Previous profile version ${profileVersion ?? "unknown"}` : `Current profile version ${profileVersion}`}</p><h2>Fit analysis</h2></div><div className="score" aria-label={`Fit score ${payload.score} out of 100`}>{payload.score}<span>/100</span></div></div>
    {isStale && <p className="stale-note">This result is from a previous profile version. Regenerate analysis to see a current assessment.</p>}
