...
export function ResumeSuggestionsView({ result }: { result: { payload: ResumeSuggestions; profileVersion: number | null; isStale: boolean; isPreviousAttempt?: boolean } | null }) {
  ...
  return <section className={`card result-card ${isStale ? "stale-result" : ""}`} aria-label={isStale ? "Previous résumé suggestions" : "Current résumé suggestions"}>
    <p className="eyebrow">{isPreviousAttempt ? `Previous analysis attempt · profile version ${profileVersion ?? "unknown"}` : isStale ? `Previous profile version ${profileVersion ?? "unknown"}` : `Current profile version ${profileVersion}`}</p><h2>Résumé suggestions</h2>
    {isStale && <p className="stale-note">{isPreviousAttempt ? "These suggestions are retained from a previous analysis attempt. Regenerate analysis before using them." : "These suggestions use a previous profile version. Regenerate analysis for current suggestions."}</p>}
    <p>{payload.summary}</p>
